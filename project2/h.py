...
from differentials import differentials_n_p_d as differentials
...
def main():
    ...
    INIT_REL_NUM_DN_DEUTERIUM = 0
    assert not np.isnan(INIT_REL_NUM_DN_NEUTRON)
    INIT_PARAMS  = np.asarray([INIT_REL_NUM_DN_NEUTRON, INIT_REL_NUM_DN_PROTON, INIT_REL_NUM_DN_DEUTERIUM])
    assert not np.isnan(INIT_PARAMS).any()
    instance = solve_ivp(differentials, [INIT_LOG_TEMP, FINAL_LOG_TEMP], INIT_PARAMS, method="Radau", rtol=1e-8, atol=1e-8)
